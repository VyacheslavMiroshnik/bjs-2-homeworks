...
function calculateTotalMortgage(percent, contribution, amount, countMonths) {
  let checkPercent =
    typeof +percent === "number" && !Number.isNaN(+percent)
      ? ""
      : `Параметр процент содержит неправильное значение ${percent} `;
  let checkContribution =
    typeof +contribution === "number" && !Number.isNaN(+contribution)
      ? ""
      : `Параметр первоначальный взнос содержит неправильное значение ${contribution} `;
  let checkAmount =
    typeof +amount === "number" && !Number.isNaN(+amount)
      ? ""
      : `Параметр сумма кредита содержит неправильное значение ${amount} `;
  let checkCountMonth =
    typeof +countMonths === "number" && !Number.isNaN(+countMonths)
      ? ""
      : `Параметр дата окончания содержит неправильное значение ${countMonths} `;
  if (checkPercent || checkContribution || checkAmount || checkCountMonth) {
    return (
      !checkPercent && !checkContribution && !checkAmount && !checkCountMonth
    );
  }
  let totalAmount;
  ...
  let payByMonth =
    clearAmount *
    (percentByMonth +
      percentByMonth / (Math.pow(1 + percentByMonth, countMonths) - 1));
